...
- **Constraints & Invariants**:
  ...
  - **Two-Tier Layout Separation (Step 0 vs Step 1)**:
    - **Step 0 (Cổng học viên / `/dashboard`, `/courses`, `/courses/:id`)**: Renders with dedicated Portal Sidebar (`Tổng quan đa môn` & `Khóa học của tôi`) on both desktop and mobile drawer. Header remains clean and minimal without central tabs. UserDropdown is focused exclusively on Account (`/profile`) and Logout.
    - **Step 1 (Không gian lớp học / `/overview`, `/homework`, etc.)**: Renders full classroom workspace sidebar with the 6 core course-specific modules (`Overview`, `Exercises`, `Vocabulary`, `Dictation`, `Course Materials`, `Course Info`). Navigation back to Step 0 reuses the Header `<ChevronLeft />` button with intelligent visual affordance:
