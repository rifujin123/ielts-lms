...
export const ListeningRunner: React.FC<ListeningRunnerProps> = ({ skillData }) => {
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden bg-slate-100">
      {/* ── Top Locked Audio Player Bar ───────────────────────────── */}
      <div className="flex items-center justify-between border-b border-slate-200 bg-white px-6 py-3 shadow-xs">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-xs font-bold text-slate-900">IELTS Listening Official Audio</div>
            <div className="text-[11px] text-slate-500">
              {currentSection.title} • {currentSection.scenario}
            </div>
          </div>
        </div>

        {/* Audio controls (Play/Pause, volume slider, locked notice) */}
        <div className="flex items-center gap-4">
          <div className="hidden sm:flex items-center gap-2 rounded-lg bg-amber-50 px-2.5 py-1 text-[11px] font-semibold text-amber-700 border border-amber-200">
            <span>Chế độ thi thật: Không thể tua lại</span>
          </div>

          <button
            type="button"
            onClick={() => setIsPlaying(!isPlaying)}
            className="btn-interactive flex h-8 items-center gap-1.5 rounded-lg bg-red-600 px-3 text-xs font-bold text-white hover:bg-red-700 shadow-xs"
          >
            {isPlaying ? (
              <>
          ...
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              className="h-1.5 w-20 cursor-pointer accent-red-600"
            />
          </div>
        </div>
      </div>
