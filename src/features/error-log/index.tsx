...
export const ErrorLogPage: React.FC = () => {
  ...
  const { errors, removeError, resetToDefault, clearErrors } = useErrorLogStore()
  ...
  const filteredErrors = useMemo(() => {
    return errors.filter((err) => {
      if (filterSkill !== 'ALL' && err.skill !== filterSkill) return false
      if (filterTrap !== 'ALL' && err.trapType !== filterTrap) return false
      return true
    })
  }, [errors, filterSkill, filterTrap])

  // Handlers
  const handleRemoveError = (id: string, e?: React.MouseEvent) => {
    e?.stopPropagation()
    removeError(id)
  ...
  const currentReviewItem: LoggedError | undefined = filteredErrors[reviewIndex]
          ...
          <div>
            <div className="flex items-center gap-2.5">
              <h1 className="text-xl sm:text-2xl font-bold text-on-surface">
                Sổ tay phân tích lỗi &amp; Bẫy đề thi
              </h1>
        ...
        <div className="flex items-center gap-2 shrink-0">
          {errors.length > 0 && (
            ...
            >
              ...
              <span>Bắt đầu ôn tập ({filteredErrors.length})</span>
            </button>
          )}
        ...
        <>
            ...
            <div>
              <div className="flex items-center justify-between text-label-sm font-semibold text-secondary mb-2">
                <span className="flex items-center gap-1.5">
                  <Layers className="h-4 w-4" /> Phân bổ các nhóm bẫy đề
                </span>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-outline-variant pb-3">
            ...
            {(filterSkill !== 'ALL' || filterTrap !== 'ALL') && (
              ...
              >
                Xóa bộ lọc (Hiện tất cả {errors.length} câu)
              </button>
            )}
          </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between pt-1 border-t border-outline-variant">
                      ...
                      <button
                        type="button"
                        onClick={(e) => handleRemoveError(err.id, e)}
          ...
          <div className="flex max-h-[90vh] w-full max-w-2xl flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl">
            ...
            <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-slate-800">
                  Câu hỏi {reviewIndex + 1} / {filteredErrors.length}
                </span>
            ...
            <div className="flex items-center justify-between border-t border-slate-100 bg-slate-50/80 px-6 py-3.5">
              <div className="flex items-center gap-2">
                ...
                <button
                  type="button"
                  disabled={reviewIndex === filteredErrors.length - 1}
                  onClick={() => {
                    setReviewIndex((prev) => prev + 1)
                    setIsAnswerRevealed(false)
                  }}
              ...
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={() => {
                    handleRemoveError(currentReviewItem.id)
                    if (reviewIndex >= filteredErrors.length - 1) {
                ...
                {reviewIndex < filteredErrors.length - 1 ? (
                  <button
                    type="button"
                    onClick={() => {
                      setReviewIndex((prev) => prev + 1)
